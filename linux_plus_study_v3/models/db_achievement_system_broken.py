#!/usr/bin/env python3
"""
Database-based Achievement System for Linux+ Study Game
...
class DBAchievementSystem:
    ...
    def __init__(self, user_id: str = 'anonymous'):
        ...
        self._user_achievement = None
        self.session_points = 0
    
    def _get_user_achievement(self) -> UserAchievement:
        """Get or create user achievement record."""
        # Always fetch fresh from database to avoid session issues
    ...
    def _save_user_achievement(self, achievement_data: UserAchievement) -> None:
        """Save user achievement data to database."""
        with get_db_session() as session:
            # Merge the changes
            session.merge(achievement_data)
            session.commit()
        
        # Update cached copy
        self._user_achievement = achievement_data
    ...
    def leaderboard(self, value: List[Dict[str, Any]]) -> None:
        ...
        user_achievement = self._get_user_achievement()
        user_achievement.leaderboard = value
    ...
    def update_points(self, points_change: int, apply_multiplier: bool = True) -> None:
        ...
        if final_points > 0:
            user_achievement = self._get_user_achievement()
            user_achievement.points_earned = (user_achievement.points_earned or 0) + final_points
            self._save_user_achievement(user_achievement)
    ...
    def clear_achievements(self) -> None:
        ...
        user_achievement = self._get_user_achievement()
        # Reset to defaults
        user_achievement.badges = []
        user_achievement.points_earned = 0
        user_achievement.days_studied = []
        user_achievement.questions_answered = 0
        user_achievement.streaks_achieved = 0
        user_achievement.perfect_sessions = 0
        user_achievement.daily_warrior_dates = []
        user_achievement.leaderboard = []
        user_achievement.survival_high_score = 0
        user_achievement.survival_high_score_xp = 0
        user_achievement.custom_achievements = []
