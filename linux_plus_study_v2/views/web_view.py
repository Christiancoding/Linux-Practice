...
class LinuxPlusStudyWeb:
    ...
    def quit_app(self):
        ...
        if self.window:
            self.window.destroy()
def create_app(quiz_controller=None, stats_controller=None):
    """
    Factory function to create Flask application with injected controllers.
    
    This function creates a standalone Flask app for the unified system,
    separate from the pywebview desktop app approach.
    
    Args:
        quiz_controller: Injected QuizController instance
        stats_controller: Injected StatsController instance
        
    Returns:
        Flask: Configured Flask application ready to run
    """
    ...
    app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'linux-plus-study-secret-key')
    ...
    # Initialize database setup for web mode
    db_manager = setup_database_for_web()
    if db_manager:
        app.config['DB_MANAGER'] = db_manager
    
    # Set up CLI playground
    cli_playground = get_cli_playground()
    
    # Store controllers in app context for route access
    app.quiz_controller = quiz_controller
    app.stats_controller = stats_controller
    ...
    @app.route('/review')
    def review_page():
        """Review missed questions and study materials."""
        return render_template('review.html')
    ...
    @app.route('/cli_playground')
    def cli_playground_page():
        """Interactive CLI practice environment."""
        return render_template('cli_playground.html')
    ...
    def api_status():
        """Get current application and quiz status."""
        try:
            if not quiz_controller:
                return jsonify({'error': 'Quiz controller not available'}), 500
                
            status = quiz_controller.get_session_status()
            return jsonify({
                'quiz_active': status['quiz_active'],
                'total_questions': len(quiz_controller.game_state.questions),
                'categories': sorted(list(quiz_controller.game_state.categories)),
                'session_score': status['session_score'],
                'session_total': status['session_total'],
                'current_streak': status['current_streak'],
                'break_reminder_due': _should_show_break_reminder(quiz_controller),
                'current_mode': status.get('current_quiz_mode', 'standard')
            })
        except Exception as e:
    ...
    def start_quiz():
        ...
        try:
            ...
            question = quiz_controller.start_quiz(mode=mode, category=category)
            ...
            app.logger.error(f"Start quiz error: {e}")
            return jsonify({'error': str(e)}), 500
    ...
    def submit_answer():
        ...
        try:
            ...
            result = quiz_controller.process_answer(data['answer'])
    ...
    def end_quiz():
        ...
        try:
            ...
            results = quiz_controller.end_quiz()
            return jsonify({
                'success': True,
                'final_score': results.get('final_score', 0),
                'questions_answered': results.get('questions_answered', 0),
                'achievements_unlocked': results.get('achievements_unlocked', [])
            })
            
        except Exception as e:
            app.logger.error(f"End quiz error: {e}")
            return jsonify({'error': str(e)}), 500
    
    # ===================== CLI PLAYGROUND ROUTES =====================
    
    @app.route('/api/cli/execute', methods=['POST'])
    def execute_cli_command():
        """Execute CLI command in safe playground environment."""
        try:
            data = request.get_json()
            ...
            # Execute command through CLI playground
            ...
            app.logger.error(f"CLI execute error: {e}")
            return jsonify({'error': str(e)}), 500
    ...
    def clear_cli_history():
        """Clear CLI command history."""
        try:
            # Reset CLI playground state if needed
    ...
    def get_stats_summary():
        ...
        try:
            if not stats_controller:
                return jsonify({'error': 'Stats controller not available'}), 500
                
            stats = stats_controller.get_comprehensive_stats()
            return jsonify({
                'success': True,
                'stats': stats
            })
    ...
    def get_achievements():
        ...
        try:
            ...
            achievements = quiz_controller.game_state.achievements
            return jsonify({
                'success': True,
    ...
    def get_question_count():
        ...
        try:
            ...
            count = len(quiz_controller.game_state.questions)
            return jsonify({
                'success': True,
                'count': count
            })
    ...
    def import_questions():
        ...
        try:
            ...
            try:
                ...
                for question_data in questions_data:
                    if _validate_question_format(question_data):
                        quiz_controller.game_state.questions.append(question_data)
                        imported_count += 1
                
                # Save updated questions
                quiz_controller.game_state.save_questions()
            ...
            app.logger.error(f"Import questions error: {e}")
            return jsonify({'error': str(e)}), 500
    ...
    def _should_show_break_reminder(quiz_controller):
        ...
        try:
            ...
            if os.path.exists(settings_file):
                ...
                break_interval = settings.get('breakReminder', 10)
                return (break_interval > 0 and 
                       hasattr(quiz_controller, 'questions_since_break') and
                       quiz_controller.questions_since_break >= break_interval)
        except:
            pass
        return False
    
    def _validate_question_format(question_data):
        """Validate question format for import."""
        required_fields = ['question', 'options', 'correct_answer', 'explanation']
        return (isinstance(question_data, dict) and 
                all(field in question_data for field in required_fields) and
                isinstance(question_data['options'], list) and
                len(question_data['options']) >= 2)
